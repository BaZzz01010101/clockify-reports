...
const DETAILS_HEADERS = ['Project', 'Description', 'Date', 'From', 'To', 'Time'];
...
export const flattenDetailedEntries = (report: ClockifyDetailedReportResponse): DetailedEntryRow[] =>
...
export const createJsonBuffer = (report: ClockifyDetailedReportResponse): Buffer =>
  Buffer.from(JSON.stringify(report, null, 2), 'utf8');

export const createCsvBuffer = (report: ClockifyDetailedReportResponse): Buffer => {
  const rows = flattenDetailedEntries(report);
  const header = EXPORT_COLUMNS.join(',');
  const lines = rows.map((row) => EXPORT_COLUMNS.map((column) => escapeCsvValue(row[column])).join(','));

  return Buffer.from([header, ...lines].join('\n'), 'utf8');
};

export const createXlsxBuffer = (report: ClockifyDetailedReportResponse): Buffer => {
  const normalizedEntries = normalizeDetailedEntries(report);
  const workbook = XLSX.utils.book_new();

  XLSX.utils.book_append_sheet(workbook, createSummarySheet(normalizedEntries), 'Summary');
  XLSX.utils.book_append_sheet(workbook, createDetailsSheet(normalizedEntries), 'Details');
  workbook.Workbook = {
    ...(workbook.Workbook ?? {}),
    Views: [{ activeTab: 0 }] as unknown as NonNullable<XLSX.WorkBook['Workbook']>['Views'],
...
const normalizeDetailedEntries = (report: ClockifyDetailedReportResponse): NormalizedDetailedEntry[] =>
  report.timeentries.map((entry) => {
...
const createDetailsSheet = (entries: NormalizedDetailedEntry[]): Worksheet => {
  ...
  sheet['!cols'] = [{ wch: 28 }, { wch: 64 }, { wch: 14 }, { wch: 10 }, { wch: 10 }, { wch: 10 }];

  for (let columnIndex = 0; columnIndex < DETAILS_HEADERS.length; columnIndex += 1) {
  ...
  for (let rowIndex = 1; rowIndex < values.length; rowIndex += 1) {
    applyCellStyle(sheet, XLSX.utils.encode_cell({ c: 0, r: rowIndex }), detailTextStyle);
    applyCellStyle(sheet, XLSX.utils.encode_cell({ c: 1, r: rowIndex }), detailTextStyle);
    applyNumberCellFormatting(sheet, XLSX.utils.encode_cell({ c: 2, r: rowIndex }), DATE_FORMAT, detailNumberCellStyle);
    applyNumberCellFormatting(sheet, XLSX.utils.encode_cell({ c: 3, r: rowIndex }), TIME_FORMAT, detailNumberCellStyle);
    applyNumberCellFormatting(sheet, XLSX.utils.encode_cell({ c: 4, r: rowIndex }), TIME_FORMAT, detailNumberCellStyle);
...
const createSummarySheet = (entries: NormalizedDetailedEntry[]): Worksheet => {
  ...
  const headerRow = [
    'Tasks',
    ...summary.dayKeys.map((dayKey) => toExcelDateSerial(summary.dayDates.get(dayKey)!)),
    'Grand Total',
  ];
  ...
  sheet['!cols'] = [
    { wch: 64 },
    ...new Array(Math.max(lastColumnIndex, 1)).fill(null).map(() => ({ width: summaryDayColumnWidth })),
  ];
  sheet['!rows'] = [
    { hpx: summaryHeaderRowHeightPx },
    ...new Array(summary.rows.length).fill(null).map(() => ({ hpx: normalRowHeightPx })),
  ];
  ...
  for (let columnIndex = 0; columnIndex <= lastColumnIndex; columnIndex += 1) {
    applyCellStyle(
      ...
      columnIndex === 0 ? summaryHeaderFirstColumnStyle : summaryHeaderCenteredStyle,
    );
  }
  ...
  for (let dayOffset = 0; dayOffset < summary.dayKeys.length; dayOffset += 1) {
    ...
    applyNumberCellFormatting(sheet, headerAddress, DATE_FORMAT, summaryHeaderCenteredStyle);
  }
  ...
  summary.rows.forEach((row, rowOffset) => {
    ...
    const labelStyle = isGrandTotalRow ? totalRowStyle : isProjectRow ? projectRowStyle : descriptionLabelStyle;
    ...
    applyCellStyle(
      ...
      isFirstBodyRow ? withBorderOverride(labelStyle, headerDividerBorder) : labelStyle,
    );
    ...
    for (let columnIndex = 1; columnIndex <= lastColumnIndex; columnIndex += 1) {
      ...
      const styledNumberCell = isFirstBodyRow ? withBorderOverride(numberStyle, headerDividerBorder) : numberStyle;
      ...
      if (typeof cell?.v === 'number') {
        applyNumberCellFormatting(sheet, address, DURATION_FORMAT, styledNumberCell);
      } else {
...
const buildSummaryStructure = (entries: NormalizedDetailedEntry[]): SummaryStructure => {
  ...
  entries.forEach((entry) => {
    ...
    const descriptionTotals = projectGroup.descriptions.get(entry.description) ?? new Map<string, number>();
  ...
  for (const [projectLabel, projectGroup] of projectMap.entries()) {
    ...
    for (const [descriptionLabel, descriptionTotals] of projectGroup.descriptions.entries()) {
      rows.push({
...
const toExcelDateSerial = (dateTime: DateTime): number =>
  (Date.UTC(dateTime.year, dateTime.month - 1, dateTime.day) - EXCEL_EPOCH_MS) / MS_PER_DAY;

const toExcelTimeSerial = (dateTime: DateTime): number =>
  (dateTime.hour * 60 * 60 + dateTime.minute * 60 + dateTime.second + dateTime.millisecond / 1000) / (24 * 60 * 60);

const toExcelDurationSerial = (durationSeconds: number): number => durationSeconds / (24 * 60 * 60);

const incrementMap = (totals: Map<string, number>, key: string, value: number): void => {
...
const applyCellStyle = (sheet: Worksheet, address: string, style: unknown): void => {
  const cell = sheet[address];
...
const applyNumberCellFormatting = (sheet: Worksheet, address: string, numberFormat: string, style: unknown): void => {
  const cell = sheet[address];
...
const withBorderOverride = <T extends { border?: unknown }>(style: T, border: unknown): T => ({
  ...style,
  border,
});

const setBlankStyledCell = (sheet: Worksheet, address: string, style: unknown): void => {
  sheet[address] = {
    t: 's',
    v: '',
    s: style,
...
const escapeCsvValue = (value: DetailedEntryRow[keyof DetailedEntryRow]): string => {
